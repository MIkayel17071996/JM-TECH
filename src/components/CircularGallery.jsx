...
class Title {
  ...
  createMesh() {
    ...
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.0 - 0.05
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          vec3 borderColor = vec3(1.0, 0.749, 0.141);

          float d = roundedBoxSDF(vUv - 0.5, vec2(0.5 - uBorderRadius), uBorderRadius);

          // Scale UV for full-image fit
          float planeAspect = uPlaneSizes.x / uPlaneSizes.y;
          float imageAspect = uImageSizes.x / uImageSizes.y;
          float scale = min(planeAspect / imageAspect, 1.0) * uImageScale;
          vec2 scaledUV = (vUv - 0.5) / scale + 0.5;

          if (d > -borderWidth && d <= 0.0) {
            gl_FragColor = vec4(borderColor, 1.0);
          } else if (d > 0.0) {
            discard;
          } else {
            // draw full image letter-boxed (transparent beyond edges)
            gl_FragColor = texture2D(tMap, scaledUV);
          }
        }
      `,
      uniforms: {
        ...
        uImageScale:  { value: 1.0 }  // full image visible
      },
      transparent: true
    });
